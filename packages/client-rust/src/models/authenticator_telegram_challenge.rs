// authentik
//
// Making authentication simple.
//
// The version of the OpenAPI document: 2026.11.0-rc1
// Contact: hello@goauthentik.io
// Generated by: https://openapi-generator.tech

use serde::{Deserialize, Serialize};

use crate::models;

/// AuthenticatorTelegramChallenge : Telegram Setup challenge
#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct AuthenticatorTelegramChallenge {
    #[serde(rename = "flow_info", skip_serializing_if = "Option::is_none")]
    pub flow_info: Option<models::ContextualFlowInfo>,
    #[serde(rename = "component", skip_serializing_if = "Option::is_none")]
    pub component: Option<String>,
    #[serde(rename = "response_errors", skip_serializing_if = "Option::is_none")]
    pub response_errors: Option<std::collections::HashMap<String, Vec<models::ErrorDetail>>>,
    #[serde(rename = "pending_user")]
    pub pending_user: String,
    #[serde(rename = "pending_user_avatar")]
    pub pending_user_avatar: String,
    #[serde(rename = "code_required", skip_serializing_if = "Option::is_none")]
    pub code_required: Option<bool>,
    #[serde(rename = "bot_username")]
    pub bot_username: String,
}

impl AuthenticatorTelegramChallenge {
    /// Telegram Setup challenge
    pub fn new(
        pending_user: String,
        pending_user_avatar: String,
        bot_username: String,
    ) -> AuthenticatorTelegramChallenge {
        AuthenticatorTelegramChallenge {
            flow_info: None,
            component: None,
            response_errors: None,
            pending_user,
            pending_user_avatar,
            code_required: None,
            bot_username,
        }
    }
}
